 ...
 * {@code com.sao.saomenu.mixin.input.MouseHandlerMixin} 在 turnPlayer 的头/尾各注入一次:头部记录玩家
 * 当前朝向,尾部把本帧产生的旋转增量转存进 {@code camYaw/camPitch} 并立刻
 * 把玩家朝向回滚——玩家的朝向从未改变,旋转全部进了自由相机;
 * {@code com.sao.saomenu.mixin.input.CameraMixin} 在 Camera.setup 末尾用自由相机角度重设相机旋转与位置。</p>
 */
