 ...
 * @param titleKey 侧卡标题的语言键;无侧卡时为 {@code null}
...
public record SaoPanel(
    ...
    public static SaoPanel of(String id, String icon, Supplier<List<MenuEntry>> items) {
        return new SaoPanel(id, icon, null, items, null);
    }

    /** 带侧卡的面板。 */
    public static SaoPanel withCard(String id, String icon, String titleKey,
                                    Supplier<List<MenuEntry>> items, SideCard sideCard) {
        return new SaoPanel(id, icon, titleKey, items, sideCard);
    }
}
