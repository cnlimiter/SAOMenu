...
import com.sao.saomenu.client.hud.SAOClockPanel;
...
import com.sao.saomenu.client.hud.SaoSkillBar;
...
public class SAOMenuScreen extends Screen implements MenuHost {
    ...
    /**
     * 世界空间菜单板({@link com.sao.saomenu.client.render.SAOMenu3DPanel})的显示强度:菜单打开/关闭动画
    ...
    private void executeItemAction(int b, MenuEntry target) {
        actionMenuOpen = false;
        if (b == 0) {
            new com.sao.saomenu.network.c2s.EquipItemC2S(target.invSlot()).sendToServer();
            playClick();
        } else if (b == 1) {
            ...
            new com.sao.saomenu.network.c2s.DropItemC2S(target.invSlot(), true).sendToServer();
            playClick();
        }
    }
